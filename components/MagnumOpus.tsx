...
import { 
    ...
    ZoomIn, ZoomOut, Maximize, Terminal, ChevronRight, Package
} from 'lucide-react';
...
const MagnumOpus: React.FC = () => {
  ...
  useEffect(() => {
      if (activeTab === 'diagram' && project?.architectureDiagram && diagramRef.current) {
          ...
          // Client-side syntax fixer
          let chart = project.architectureDiagram;
          // Fix unquoted parentheses in labels: Node[Label (Detail)] -> Node["Label (Detail)"]
          chart = chart.replace(/\[([^"\]]*?\([^"\]]*?\)[^"\]]*?)\]/g, '["$1"]');
          // Ensure graph definition is clean
          chart = chart.replace(/```mermaid/g, '').replace(/```/g, '').trim();
  ...
  const downloadProject = () => {
      ...
      // Generate a simple shell script to recreate the file structure
      let script = `#!/bin/bash\n\n# Generated by Carrieder Architect\n# Project: ${project.title}\n\necho "Initializing ${project.title}..."\n\n`;
      ...
      project.codeFiles.forEach(file => {
          ...
          script += `cat << 'EOF' > "${file.name}"\n${file.content}\nEOF\n\n`;
      });
      
      script += `echo "Project scaffolding complete."\n`;

      const blob = new Blob([script], { type: 'text/x-shellscript' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `install_${project.id}.sh`;
      link.click();
  };
            ...
            {project && (
                <div className="ml-auto flex gap-3">
                    <button 
                        ...
                        className="px-4 py-2 bg-exo-primary hover:bg-exo-primary/80 text-black font-bold text-xs uppercase tracking-widest rounded-lg flex items-center gap-2 shadow-lg"
                    >
                        <Package className="w-4 h-4" /> Download Project
                    </button>
                </div>
            )}
        </div>
                    ...
                    <>
                        <div className="flex items-center justify-between border-b border-white/10 bg-black/40 backdrop-blur-md relative z-10 px-2">
                            <div className="flex">
                                ...
                                >
                                    <FileText className="w-3 h-3" /> Technical Doc
                                </button>
                                <button 
